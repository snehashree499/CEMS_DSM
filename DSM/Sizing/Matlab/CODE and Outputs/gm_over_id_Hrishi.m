...
title('Width selection of nmos for appropriate length (0.6u)')
xlabel('g_m/I_D (S/A)')
...
title('Width selection of pmos for appropriate length (0.4u)')
xlabel('g_m/I_D (S/A)')
ylabel('I_D/W (\muA/\mum)')
xlim([3 25]);
ylim([1e-2 2e2]);
...
% I_D/W vs. V_G_S and I_D/W vs. V_G_S - V_t
idw_n_vgs = 1e6*look_up(nch, 'ID_W', 'VDS', 1.2, 'VGS', nch.VGS, 'L', 0.13);
idw_p_vgs = 1e6*look_up(pch, 'ID_W', 'VDS', 1.2, 'VGS', pch.VGS, 'L', 0.13);
vt_n = look_up(nch, 'VT', 'VDS', 1.2, 'VGS', nch.VGS, 'L', 0.13);
vt_p = look_up(pch, 'VT', 'VDS', 1.2, 'VGS', pch.VGS, 'L', 0.13);

figure;
plot(nch.VGS, idw_n_vgs, pch.VGS, idw_p_vgs)
title('L=0.13\mum, I_D/W vs V_G_S')
xlabel('V_G_S (V)')
ylabel('I_D/W (\muA/\mum)')
grid;
legend('lv-nmos', 'lv-pmos', 'location', 'northeast', 'interpreter', 'none')
saveas(gcf, 'ID_W-VGS.svg')

figure;
plot(nch.VGS - vt_n, idw_n_vgs, pch.VGS - vt_p, idw_p_vgs)
title('L=0.13\mum, I_D/W vs V_G_S - V_t')
xlabel('V_G_S - V_t (V)')
ylabel('I_D/W (\muA/\mum)')
grid;
legend('lv-nmos', 'lv-pmos', 'location', 'northeast', 'interpreter', 'none')
saveas(gcf, 'ID_W-VGS-Vt.svg')
